...
//1.js的分层(功能) : jquery(tools)  封装的组件(ui)  应用(app), mvc(backboneJs)
...
mv.ui.fadeIn=function(obj){
	...
	obj.timer=setInterval (function(){
		var iSpeed=5;//透明度每次递增5
		if (value==100) {
	      clearInterval(obj.timer);//定时器停下
	  }
	  else{
	  	value+=iSpeed;
...
mv.app.toBanner=function  () {
	...
	function auto(){//往后走
		...
		for(var i=0;i<aLi.length;i++){
			mv.ui.fadeOut(aLi[i]);//所有li淡出
		}
		mv.ui.fadeIn(aLi[iNow]);//淡入当前的
	}
	function autoPrev(){//往前走
		if(iNow == 0){
			iNow = aLi.length-1;//0的下一个是aLi.length-1
		}
		else{
			iNow--;
		}
	...
	for(var i=0;i<aDd.length;i++){
		...
		aDd[i].onclick = function(ev){
	 	...
	 	document.onclick = function(){
			...
			ev.cancelBubble=true;//阻止冒泡，cancelBubble在新版本chrome,opera浏览器中已经支持
		}
	}
...
mv.app.toRun = function(){
	...
	oUl.style.width = aLi.length * aLi[0].offsetWidth + 'px';//offsetWidth=border+padding+height;
